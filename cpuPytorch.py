...
def generate_BC_training_data(length, height, step):
    ...
    x_boundary = torch.cat([
        torch.cat([torch.linspace(0, length, step)[1:step-1, None], torch.zeros((step-2, 1))], dim=1),  # 下边界
        torch.cat([torch.linspace(0, length, step)[1:step-1, None], height * torch.ones((step-2, 1))], dim=1),  # 上边界
    ...
    y_boundary = torch.zeros((step, 2))  # 应力载荷条件的大小
    y_boundary[:, 0] = torch.linspace(5, 0, step)[:step]
...
def train(maxiters, n, num_phi_train, step):
    ...
    for i in range(n):
        ...
        if i < n:
            x_interior = torch.rand((train_interior, 2))  # 随机生成内部点
            x_interior[:, 0] *= length
            x_interior[:, 1] *= height
        ...
        #     x_interior = torch.rand((final_interior, 2))  # 随机生成内部点
        #     x_interior[:, 0] *= length
        #     x_interior[:, 1] *= height
        #     x_interior.requires_grad_(True)
        #
...
def plot_results(model, loss_history):
    ...
    calculate_sigma_xx = calculate_sigma_xx.reshape(1000, 1000).detach().numpy()
    calculate_sigma_yy = calculate_sigma_yy.reshape(1000, 1000).detach().numpy()
    calculate_sigma_xy = calculate_sigma_xy.reshape(1000, 1000).detach().numpy()

    fxx_x = fxx_x.reshape(1000, 1000).detach().numpy()
    fxy_x = fxy_x.reshape(1000, 1000).detach().numpy()
    fxy_y = fxy_y.reshape(1000, 1000).detach().numpy()
    fyy_y = fyy_y.reshape(1000, 1000).detach().numpy()

    fxx_x_calculate = fxx_x_calculate.reshape(1000, 1000).detach().numpy()
    fxy_x_calculate = fxy_x_calculate.reshape(1000, 1000).detach().numpy()
    fxy_y_calculate = fxy_y_calculate.reshape(1000, 1000).detach().numpy()
    fyy_y_calculate = fyy_y_calculate.reshape(1000, 1000).detach().numpy()
