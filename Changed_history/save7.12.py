...
def normalize_date(data):
    ...
    data_normalized.requires_grad_(True)
...
def generate_BC_training_data(length, height, step):
    ...
    y_boundary[:, 0] = torch.linspace(1, 0, step, device=device)[:step]
...
def loss_fn(model, step, x_interior, x_boundary_fixed, y_boundary_fixed, x_boundary, y_boundary):
    ...
    # 计算PDE损失
    x_interior_loss_total = normalize_date(x_interior_loss_total)
    ...
    # epsilon_sigma = 1
    # epsilon_sigma_tensor = torch.tensor(epsilon_sigma ** 2, device=device)

    phy_loss = (torch.mean((sigma_xx_pred - sigma_xx) ** 2) + \
                        torch.mean((sigma_xy_pred - sigma_xy) ** 2) + \
                        torch.mean((sigma_yy_pred - sigma_yy) ** 2))

    x_interior_without_F = normalize_date(x_interior_without_F)
    ...
    # without F 平衡方程
    fxx_x = torch.autograd.grad(sigma_xx_pred_without_F, x_interior_without_F,
                                grad_outputs=torch.ones_like(sigma_xx_pred_without_F), create_graph=True)[0][:, 0]
    fxy_x = torch.autograd.grad(sigma_xy_pred_without_F, x_interior_without_F,
                                grad_outputs=torch.ones_like(sigma_xy_pred_without_F), create_graph=True)[0][:, 0]
    fxy_y = torch.autograd.grad(sigma_xy_pred_without_F, x_interior_without_F,
                                grad_outputs=torch.ones_like(sigma_xy_pred_without_F), create_graph=True)[0][:, 1]
    fyy_y = torch.autograd.grad(sigma_yy_pred_without_F, x_interior_without_F,
                                grad_outputs=torch.ones_like(sigma_yy_pred_without_F), create_graph=True)[0][:, 1]

    fxx_x_cal = torch.autograd.grad(sigma_xx_without_F, x_interior_without_F,
                                grad_outputs=torch.ones_like(sigma_xx_pred_without_F), create_graph=True)[0][:, 0]
    fxy_x_cal = torch.autograd.grad(sigma_xy_without_F, x_interior_without_F,
                                grad_outputs=torch.ones_like(sigma_xy_pred_without_F), create_graph=True)[0][:, 0]
    fxy_y_cal = torch.autograd.grad(sigma_xy_without_F, x_interior_without_F,
                                grad_outputs=torch.ones_like(sigma_xy_pred_without_F), create_graph=True)[0][:, 1]
    fyy_y_cal = torch.autograd.grad(sigma_yy_without_F, x_interior_without_F,
                                grad_outputs=torch.ones_like(sigma_yy_pred_without_F), create_graph=True)[0][:, 1]

    # 平衡方程损失
    epsilon_f = 0.01
    epsilon_f_tensor = torch.tensor(epsilon_f ** 2, device=device)

    balence_without_F_loss = (torch.mean((fxx_x + fxy_y) ** 2) +
                              torch.mean((fxy_x + fyy_y) ** 2))

    balence_without_F_calculate_loss = (torch.mean((fxx_x_cal + fxy_y_cal) ** 2) +
                              torch.mean((fxy_x_cal + fyy_y_cal) ** 2))

    # 边界载荷损失
    x_interior_right = normalize_date(x_interior_right)
    ...
    sigma_xx_right, sigma_yy_right, sigma_xy_right = calculate_sigma(pred_right[:, 0], pred_right[:, 1], x_interior_right)

    fxx_x_right = torch.autograd.grad(sigma_xx_pred_right, x_interior_right,
                                grad_outputs=torch.ones_like(sigma_xx_pred_right), create_graph=True)[0][:, 0]
    fxy_x_right = torch.autograd.grad(sigma_xy_pred_right, x_interior_right,
                                grad_outputs=torch.ones_like(sigma_xy_pred_right), create_graph=True)[0][:, 0]
    fxy_y_right = torch.autograd.grad(sigma_xy_pred_right, x_interior_right,
                                grad_outputs=torch.ones_like(sigma_xy_pred_right), create_graph=True)[0][:, 1]
    fyy_y_right = torch.autograd.grad(sigma_yy_pred_right, x_interior_right,
                                grad_outputs=torch.ones_like(sigma_yy_pred_right), create_graph=True)[0][:, 1]

    fxx_x_cal_right = torch.autograd.grad(sigma_xx_right, x_interior_right,
                                      grad_outputs=torch.ones_like(sigma_xx_right), create_graph=True)[0][:, 0]
    fxy_x_cal_right = torch.autograd.grad(sigma_xy_right, x_interior_right,
                                      grad_outputs=torch.ones_like(sigma_xy_right), create_graph=True)[0][:, 0]
    fxy_y_cal_right = torch.autograd.grad(sigma_xy_right, x_interior_right,
                                      grad_outputs=torch.ones_like(sigma_xy_right), create_graph=True)[0][:, 1]
    fyy_y_cal_right = torch.autograd.grad(sigma_yy_right, x_interior_right,
                                      grad_outputs=torch.ones_like(sigma_yy_right), create_graph=True)[0][:, 1]


    balence_F_loss = (torch.mean((fxx_x_right + fxy_y_right - y_boundary[:, 0]) ** 2)+
                      torch.mean((fxy_x_right + fyy_y_right - y_boundary[:, 1]) ** 2))

    balence_F_calculate_loss = (torch.mean((fxx_x_cal_right + fxy_y_cal_right - y_boundary[:, 0]) ** 2) +
                      torch.mean((fxy_x_cal_right + fyy_y_cal_right - y_boundary[:, 1]) ** 2))

    # 左侧固定端 施加位移约束
    x_boundary_fixed = normalize_date(x_boundary_fixed)
    fixed_pred = model(x_boundary_fixed)
    fixed_pred.requires_grad_(True)

    # 固定段条件损失
    u_pred_fixed, v_pred_fixed = fixed_pred[:, 0], fixed_pred[:, 1]

    epsilon_u = 1 * 1e-7
    epsilon_u_tensor = torch.tensor(epsilon_u ** 2, device=device)

    # fixed_loss = (torch.mean(((u_pred_fixed - y_boundary_fixed[:, 0]) ** 2)/torch.max(u_pred_fixed ** 2, epsilon_u_tensor)) +
    #               torch.mean(((v_pred_fixed - y_boundary_fixed[:, 1]) ** 2)/torch.max(v_pred_fixed ** 2, epsilon_u_tensor)))

    fixed_loss = (torch.mean(((u_pred_fixed) ** 2)/torch.max(u_pred_fixed ** 2, epsilon_u_tensor)) +
                  torch.mean(((v_pred_fixed) ** 2)/torch.max(v_pred_fixed ** 2, epsilon_u_tensor)))
    ...
    PINN_loss = (lambda_balence * balence_without_F_loss +
                 lambda_balence * balence_without_F_calculate_loss +
                 lambda_fixed * fixed_loss +
                 lambda_BC * balence_F_loss +
                 lambda_BC * balence_F_calculate_loss +
                 lambda_phy * phy_loss)

    return (PINN_loss, balence_without_F_loss, fixed_loss, balence_F_loss, phy_loss,
            balence_without_F_calculate_loss, balence_F_calculate_loss)
        ...
        if i < n :
            ...
            for epoch in range(steps_count):
                ...
                if epoch % num_print_epoch == 0:
                    print(f'Iteration {i + 1}/{n}, Epoch {epoch}, Loss: {loss.item():.8f}, '
                          f'balence_without_F_loss: {balence_without_F_loss.item():.8f}, '
                          f'fixed_loss: {fixed_loss.item():.8f}, '
                          f'balence_F_loss: {balence_F_loss.item():.8f}, '
                          f'phy_loss: {phy_loss.item():.8f}, '
                          f'balence_without_F_calculate_loss: {balence_without_F_calculate_loss.item():.8f}, '
                          f'balence_F_calculate_loss: {balence_F_calculate_loss.item():.8f}, ')
...
def plot_results(model, loss_history):
    ...
    xy_tensor = normalize_date(xy_tensor)
    ...
    # # 绘制应变云图 u_x
    # plt.figure()
    # plt.imshow(u_x, extent=(0, 20, 0, 5), origin='lower', cmap='jet')
    # plt.colorbar()
    # plt.title('u_x')
    # plt.xlabel('Length')
    # plt.ylabel('Height')
    ...
    # # 绘制应变云图 u_y
    # plt.figure()
    # plt.imshow(u_y, extent=(0, 20, 0, 5), origin='lower', cmap='jet')
    # plt.colorbar()
    # plt.title('u_y')
    # plt.xlabel('Length')
    # plt.ylabel('Height')
    ...
    # # 绘制应变云图 v_x
    # plt.figure()
    # plt.imshow(v_x, extent=(0, 20, 0, 5), origin='lower', cmap='jet')
    # plt.colorbar()
    # plt.title('v_x')
    # plt.xlabel('Length')
    # plt.ylabel('Height')
    ...
    # # 绘制应变云图 v_y
    # plt.figure()
    # plt.imshow(v_y, extent=(0, 20, 0, 5), origin='lower', cmap='jet')
    # plt.colorbar()
    # plt.title('v_y')
    # plt.xlabel('Length')
    # plt.ylabel('Height')
    ...
    # # 绘制应力云图 calculate_sigma_xx
    # plt.figure()
    # plt.imshow(calculate_sigma_xx, extent=(0, 20, 0, 5), origin='lower', cmap='jet')
    # plt.colorbar()
    # plt.title('Calculated Stress $\sigma_{xx}$')
    # plt.xlabel('Length')
    # plt.ylabel('Height')
    # plt.show()
    #
    # # 绘制应力云图 calculate_sigma_yy
    # plt.figure()
    # plt.imshow(calculate_sigma_yy, extent=(0, 20, 0, 5), origin='lower', cmap='jet')
    # plt.colorbar()
    # plt.title('Calculated Stress $\sigma_{yy}$')
    # plt.xlabel('Length')
    # plt.ylabel('Height')
    # plt.show()
    #
    # # 绘制应力云图 calculate_sigma_xy
    # plt.figure()
    # plt.imshow(calculate_sigma_xy, extent=(0, 20, 0, 5), origin='lower', cmap='jet')
    # plt.colorbar()
    # plt.title('Calculated Stress $\sigma_{xy}$')
    # plt.xlabel('Length')
    # plt.ylabel('Height')
    # plt.show()
