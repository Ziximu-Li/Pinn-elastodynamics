...
def normalize_to_01(data):
    data_min = data.min()
    data_max = data.max()
    data_normalized = (data - data_min) / (data_max - data_min + 1e-9)  # 加上一个小的常数以避免除零
    return data_normalized
...
def loss_fn(model, step, x_interior, x_boundary_fixed, y_boundary_fixed, x_boundary, y_boundary):
    ...
    return PINN_loss, balence_without_F_loss, fixed_loss, balence_F_loss, phy_loss, sigma_loss, loss_fixed_f
    ...
    for i in range(n):
        ...
        if i < n :
            ...
            for epoch in range(steps_count):
                optimizer.zero_grad()
                loss, balence_without_F_loss, fixed_loss, balence_F_loss, phy_loss, sigma_loss, loss_fixed_f\
                ...
                if epoch % num_print_epoch == 0:
                    print(f'Iteration {i + 1}/{n}, Epoch {epoch}, Loss: {loss.item():.8f}, '
                          ...
                          f'sigma_loss: {sigma_loss.item():.8f}, '
                          f'loss_fixed_f: {loss_fixed_f.item():.8f}')
